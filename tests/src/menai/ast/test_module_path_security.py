...
class TestValidPaths:
    ...
    def test_import_from_subdirectory(self, tmp_path):
        ...
        module_file = lib_dir / "utils.menai"
        module_file.write_text("(let ((square (lambda (x) (integer* x x)))) (export square))")
...
class TestModulePathUpdate:
    ...
    def test_set_module_path_allows_reload_from_new_location(self, tmp_path):
        ...
        (dir1 / "test.menai").write_text("(let ((value 1)) (export value))")
        (dir2 / "test.menai").write_text("(let ((value 2)) (export value))")
...
class TestCacheClearingBehavior:
    ...
    def test_clear_module_cache_removes_all_modules(self, tmp_path):
        ...
        (tmp_path / "mod1.menai").write_text("(let ((x 1)) (export x))")
        (tmp_path / "mod2.menai").write_text("(let ((x 2)) (export x))")
        (tmp_path / "mod3.menai").write_text("(let ((x 3)) (export x))")
...
class TestSecurityScenarios:
    ...
    def test_cannot_escape_module_path_with_absolute(self, tmp_path):
        ...
        allowed_dir.mkdir()
        (allowed_dir / "safe.menai").write_text("(dict \"x\" 1)")
    ...
    def test_cannot_escape_module_path_with_parent_nav(self, tmp_path):
        ...
        (forbidden_dir / "secret.menai").write_text("(dict \"secret\" 42)")
