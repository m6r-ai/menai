...
class TestDictConstruction:
    ...
    def test_dict_preserves_insertion_order(self, tool):
        ...
        result = tool.evaluate('(dict "z" 1 "a" 2 "m" 3)')
...
class TestDictConstructionErrors:
    ...
    def test_dict_odd_args(self, tool):
        """Test compile-time error when dict has an odd number of arguments."""
        with pytest.raises(MenaiEvalError, match="even number of arguments"):
            tool.evaluate('(dict "key")')

    def test_dict_three_args(self, tool):
        """Test compile-time error with three arguments."""
        with pytest.raises(MenaiEvalError, match="even number of arguments"):
            tool.evaluate('(dict "a" 1 "b")')
...
class TestDictHas:
    ...
    def test_dict_has_existing_key(self, tool):
        ...
        result = tool.evaluate('(dict-has? (dict "name" "Alice" "age" 30) "name")')
        assert result is True

    def test_dict_has_missing_key(self, tool):
        """Test checking for missing key."""
        result = tool.evaluate('(dict-has? (dict "name" "Alice") "age")')
...
class TestDictHasErrors:
    ...
    def test_dict_has_wrong_arg_count(self, tool):
        """Test error with wrong number of arguments."""
        with pytest.raises(MenaiEvalError, match="wrong number of arguments"):
            tool.evaluate('(dict-has? (dict "a" 1))')
...
class TestDictKeys:
    ...
    def test_dict_keys_simple(self, tool):
        ...
        result = tool.evaluate('(dict-keys (dict "name" "Alice" "age" 30 "city" "NYC"))')
    ...
    def test_dict_keys_preserves_order(self, tool):
        ...
        result = tool.evaluate('(dict-keys (dict "z" 1 "a" 2 "m" 3))')
        assert result == ["z", "a", "m"]
...
class TestDictValues:
    ...
    def test_dict_values_simple(self, tool):
        ...
        result = tool.evaluate('(dict-values (dict "name" "Alice" "age" 30 "city" "NYC"))')
    ...
    def test_dict_values_preserves_order(self, tool):
        ...
        result = tool.evaluate('(dict-values (dict "z" 1 "a" 2 "m" 3))')
        assert result == [1, 2, 3]
...
class TestDictValuesErrors:
    ...
    def test_dict_values_wrong_arg_count(self, tool):
        """Test error with wrong number of arguments."""
        with pytest.raises(MenaiEvalError, match="wrong number of arguments"):
            tool.evaluate('(dict-values (dict "a" 1) "extra")')
...
class TestDictMergeErrors:
    ...
    def test_dict_merge_wrong_arg_count(self, tool):
        """Test error with wrong number of arguments."""
        with pytest.raises(MenaiEvalError, match="wrong number of arguments"):
            tool.evaluate('(dict-merge (dict "a" 1))')
...
class TestDictPredicate:
    ...
    def test_dict_predicate_true(self, tool):
        ...
        result = tool.evaluate('(dict? (dict "name" "Alice"))')
        assert result is True
...
class TestDictFormatting:
    ...
    def test_dict_format_simple(self, tool):
        ...
        result = tool.evaluate_and_format('(dict "name" "Alice" "age" 30)')
        assert result == '{("name" "Alice") ("age" 30)}'
    ...
    def test_dict_format_nested(self, tool):
        ...
        result = tool.evaluate_and_format('(dict "items" (list 1 2 3))')
...
class TestDictPatternMatching:
    ...
    def test_match_with_dict_operations(self, tool):
        ...
        result = tool.evaluate('''
            (match (dict "type" "user" "name" "Alice")
              ((? dict? data)
...
class TestDictComplexScenarios:
    ...
    def test_merge_multiple_dicts(self, tool):
        ...
        result = tool.evaluate('''
            (let* ((defaults (dict "port" 8080 "host" "localhost"))
                  (config (dict "port" 3000))
                  (overrides (dict "debug" #t)))
...
class TestDictEquality:
    ...
    def test_dict_equality_same_content(self, tool):
        ...
        result = tool.evaluate('(dict=? (dict "a" 1 "b" 2) (dict "a" 1 "b" 2))')
        assert result is True

    def test_dict_equality_different_content(self, tool):
        """Test that dicts with different content are not equal."""
        result = tool.evaluate('(dict=? (dict "a" 1 "b" 2) (dict "a" 1 "b" 3))')
        assert result is False
...
class TestDictLength:
    ...
    def test_length_with_dict(self, tool):
        ...
        result = tool.evaluate('(dict-length (dict "name" "Alice" "age" 30 "city" "NYC"))')
        assert result == 3
    ...
    def test_length_single_entry_dict(self, tool):
        ...
        result = tool.evaluate('(dict-length (dict "only" "one"))')
        assert result == 1

    def test_length_with_nested_dict(self, tool):
        """Test length of dict containing nested dicts."""
        result = tool.evaluate('''
            (dict-length (dict "user" (dict "name" "Bob" "id" 123)))
        ''')
        assert result == 1
...
class TestDictLengthFunction:
    ...
    def test_dict_length_basic(self, tool):
        ...
        result = tool.evaluate('(dict-length (dict "x" 1 "y" 2))')
        assert result == 2
...
class TestDictFirstClass:
    ...
    def test_dict_passed_to_higher_order_function(self, tool):
        ...
        result = tool.evaluate(
            '(map-list (lambda (pair) (dict (list-first pair) (list-first (list-rest pair))))'
    ...
    def test_dict_returned_from_function(self, tool):
        ...
        result = tool.evaluate(
            ...
            '  ((get-constructor "dict") "k" 42))'
        )
        assert result == {"k": 42}

    def test_dict_called_via_apply(self, tool):
        """dict can be invoked through apply with a flat key-value argument list."""
        result = tool.evaluate(
            '(apply dict (list "a" 1 "b" 2 "c" 3))'
        )
    ...
    def test_dict_called_via_apply_odd_args_runtime_error(self, tool):
        """apply with dict and an odd number of arguments raises a runtime error."""
        with pytest.raises(MenaiEvalError, match="even number of arguments"):
            tool.evaluate('(apply dict (list "a" 1 "b"))')
