...
class TestDictMap:
    ...
    def test_map_returns_dict(self, tool):
        """Test that map-dict returns an dict, not a list."""
        result = tool.evaluate(
...
class TestDictFilter:
    ...
    def test_filter_single_entry_kept(self, tool):
        ...
        result = tool.evaluate(
            '(filter-dict (lambda (k v) #t) (dict (list "only" 42)))'
        )
    ...
    def test_filter_single_entry_removed(self, tool):
        ...
        result = tool.evaluate(
            '(filter-dict (lambda (k v) #f) (dict (list "only" 42)))'
        )
        assert result == {}
    ...
    def test_filter_returns_dict(self, tool):
        """Test that filter-dict returns an dict, not a list."""
        result = tool.evaluate(
