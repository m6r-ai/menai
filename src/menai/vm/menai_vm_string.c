 ...
 * (menai_vm_bridge.c) via alloc_menai_string_from_utf8 / alloc_utf8_from_menai_string.
 */
#include <math.h>
#include <stdint.h>
#include <stdio.h>
...
ssize_t
menai_string_upcase_length(MenaiString *s)
{
    ssize_t len = s->length;
...
void
menai_string_upcase(MenaiString *s, MenaiString *r)
{
    ssize_t len = s->length;
    ssize_t k = 0;
    for (ssize_t i = 0; i < len; i++) {
        ...
        if (exp) {
            for (int j = 0; j < 3 && exp->expansion[j]; j++) {
                r->data[k++] = exp->expansion[j];
            }
        } else {
            r->data[k++] = unicode_simple_upcase(s->data[i]);
        }
    }
}

void
menai_string_downcase(MenaiString *s, MenaiString *r)
{
    ssize_t len = s->length;
    for (ssize_t i = 0; i < len; i++) {
        r->data[i] = unicode_simple_downcase(s->data[i]);
    }
}
...
/*
 * alloc_menai_string_from_float — format a double as a Menai string, matching Python's
...
/*
 * alloc_menai_string_from_complex — format a complex number as a Menai string, matching
