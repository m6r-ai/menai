...
class Opcode(IntEnum):
    ...
    PATCH_CLOSURE = _op(14, 3)          # r_src0.captures[src1] = r_src2
    CALL = _op(15, 2)                   # r_dest = CALL r_src0 src1 — func in src0, arity in src1, result to dest
    TAIL_CALL = _op(16, 2)              # TAIL_CALL r_src0 src1 — func in src0, arity in src1, no dest
    APPLY = _op(17, 2)                  # r_dest = APPLY r_src0 r_src1 — func in src0, arg_list reg in src1, result to dest
    TAIL_APPLY = _op(18, 2)             # TAIL_APPLY r_src0 r_src1 — func in src0, arg_list reg in src1, no dest
    RETURN = _op(19, 1)                 # RETURN r_src0 — return value in r_src0

    # None operations
    NONE_P = _op(20, 1)                 # r_dest = (none? r_src0)

    # Function operations
    FUNCTION_P = _op(21, 1)             # r_dest = (function? r_src0)
    FUNCTION_EQ_P = _op(22, 2)          # r_dest = (function=? r_src0 r_src1)
    FUNCTION_NEQ_P = _op(23, 2)         # r_dest = (function!=? r_src0 r_src1)
    FUNCTION_MIN_ARITY = _op(24, 1)     # r_dest = (function-min-arity r_src0)
    FUNCTION_VARIADIC_P = _op(25, 1)    # r_dest = (function-variadic? r_src0)
    FUNCTION_ACCEPTS_P = _op(26, 2)     # r_dest = (function-accepts? r_src0 r_src1)

    # Symbol operations
    SYMBOL_P = _op(27, 1)               # r_dest = (symbol? r_src0)
    SYMBOL_EQ_P = _op(28, 2)            # r_dest = (symbol=? r_src0 r_src1)
    SYMBOL_NEQ_P = _op(29, 2)           # r_dest = (symbol!=? r_src0 r_src1)
    SYMBOL_TO_STRING = _op(30, 1)       # r_dest = (symbol->string r_src0)

    # Boolean operations
    BOOLEAN_P = _op(31, 1)             # r_dest = (boolean? r_src0)
    BOOLEAN_EQ_P = _op(32, 2)          # r_dest = (boolean=? r_src0 r_src1)
    BOOLEAN_NEQ_P = _op(33, 2)         # r_dest = (boolean!=? r_src0 r_src1)
    BOOLEAN_NOT = _op(34, 1)           # r_dest = (boolean-not r_src0)

    # Integer operations
    INTEGER_P = _op(35, 1)             # r_dest = (integer? r_src0)
    INTEGER_EQ_P = _op(36, 2)          # r_dest = (integer=? r_src0 r_src1)
    INTEGER_NEQ_P = _op(37, 2)         # r_dest = (integer!=? r_src0 r_src1)
    INTEGER_LT_P = _op(38, 2)          # r_dest = (integer<? r_src0 r_src1)
    INTEGER_GT_P = _op(39, 2)          # r_dest = (integer>? r_src0 r_src1)
    INTEGER_LTE_P = _op(40, 2)         # r_dest = (integer<=? r_src0 r_src1)
    INTEGER_GTE_P = _op(41, 2)         # r_dest = (integer>=? r_src0 r_src1)
    INTEGER_ABS = _op(42, 1)           # r_dest = (integer-abs r_src0)
    INTEGER_ADD = _op(43, 2)           # r_dest = (integer+ r_src0 r_src1)
    INTEGER_SUB = _op(44, 2)           # r_dest = (integer- r_src0 r_src1)
    INTEGER_MUL = _op(45, 2)           # r_dest = (integer* r_src0 r_src1)
    INTEGER_DIV = _op(46, 2)           # r_dest = (integer/ r_src0 r_src1)
    INTEGER_MOD = _op(47, 2)           # r_dest = (integer% r_src0 r_src1)
    INTEGER_NEG = _op(48, 1)           # r_dest = (integer-neg r_src0)
    INTEGER_EXPN = _op(49, 2)          # r_dest = (integer-expn r_src0 r_src1)
    INTEGER_BIT_NOT = _op(50, 1)       # r_dest = (integer-bit-not r_src0)
    INTEGER_BIT_SHIFT_LEFT = _op(51, 2)
                                        # r_dest = (integer-bit-shift-left r_src0 r_src1)
    INTEGER_BIT_SHIFT_RIGHT = _op(52, 2)
                                        # r_dest = (integer-bit-shift-right r_src0 r_src1)
    INTEGER_BIT_OR = _op(53, 2)        # r_dest = (integer-bit-or r_src0 r_src1)
    INTEGER_BIT_AND = _op(54, 2)       # r_dest = (integer-bit-and r_src0 r_src1)
    INTEGER_BIT_XOR = _op(55, 2)       # r_dest = (integer-bit-xor r_src0 r_src1)
    INTEGER_MIN = _op(56, 2)           # r_dest = (integer-min r_src0 r_src1)
    INTEGER_MAX = _op(57, 2)           # r_dest = (integer-max r_src0 r_src1)
    INTEGER_TO_FLOAT = _op(58, 1)      # r_dest = (integer->float r_src0)
    INTEGER_TO_COMPLEX = _op(59, 2)    # r_dest = (integer->complex r_src0 r_src1)
    INTEGER_TO_STRING = _op(60, 2)     # r_dest = (integer->string r_src0 r_src1)
    INTEGER_CODEPOINT_TO_STRING = _op(61, 1)
                                        # r_dest = (integer-codepoint->string r_src0)

    # Floating point operations
    FLOAT_P = _op(62, 1)               # r_dest = (float? r_src0)
    FLOAT_EQ_P = _op(63, 2)            # r_dest = (float=? r_src0 r_src1)
    FLOAT_NEQ_P = _op(64, 2)           # r_dest = (float!=? r_src0 r_src1)
    FLOAT_LT_P = _op(65, 2)            # r_dest = (float<? r_src0 r_src1)
    FLOAT_GT_P = _op(66, 2)            # r_dest = (float>? r_src0 r_src1)
    FLOAT_LTE_P = _op(67, 2)           # r_dest = (float<=? r_src0 r_src1)
    FLOAT_GTE_P = _op(68, 2)           # r_dest = (float>=? r_src0 r_src1)
    FLOAT_NEG = _op(69, 1)             # r_dest = (float-neg r_src0)
    FLOAT_ADD = _op(70, 2)             # r_dest = (float+ r_src0 r_src1)
    FLOAT_SUB = _op(71, 2)             # r_dest = (float- r_src0 r_src1)
    FLOAT_MUL = _op(72, 2)             # r_dest = (float* r_src0 r_src1)
    FLOAT_DIV = _op(73, 2)             # r_dest = (float/ r_src0 r_src1)
    FLOAT_FLOOR_DIV = _op(74, 2)       # r_dest = (float// r_src0 r_src1)
    FLOAT_MOD = _op(75, 2)             # r_dest = (float% r_src0 r_src1)
    FLOAT_EXP = _op(76, 1)             # r_dest = (float-exp r_src0)
    FLOAT_EXPN = _op(77, 2)            # r_dest = (float-expn r_src0 r_src1)
    FLOAT_LOG = _op(78, 1)             # r_dest = (float-log r_src0)
    FLOAT_LOG10 = _op(79, 1)           # r_dest = (float-log10 r_src0)
    FLOAT_LOG2 = _op(80, 1)            # r_dest = (float-log2 r_src0)
    FLOAT_LOGN = _op(81, 2)            # r_dest = (float-logn r_src0 r_src1)
    FLOAT_SIN = _op(82, 1)             # r_dest = (float-sin r_src0)
    FLOAT_COS = _op(83, 1)             # r_dest = (float-cos r_src0)
    FLOAT_TAN = _op(84, 1)             # r_dest = (float-tan r_src0)
    FLOAT_SQRT = _op(85, 1)            # r_dest = (float-sqrt r_src0)
    FLOAT_ABS = _op(86, 1)             # r_dest = (float-abs r_src0)
    FLOAT_TO_INTEGER = _op(87, 1)      # r_dest = (float->integer r_src0)
    FLOAT_TO_COMPLEX = _op(88, 2)      # r_dest = (float->complex r_src0 r_src1)
    FLOAT_TO_STRING = _op(89, 1)       # r_dest = (float->string r_src0)
    FLOAT_FLOOR = _op(90, 1)           # r_dest = (float-floor r_src0)
    FLOAT_CEIL = _op(91, 1)            # r_dest = (float-ceil r_src0)
    FLOAT_ROUND = _op(92, 1)           # r_dest = (float-round r_src0)
    FLOAT_MIN = _op(93, 2)             # r_dest = (float-min r_src0 r_src1)
    FLOAT_MAX = _op(94, 2)             # r_dest = (float-max r_src0 r_src1)
    FLOAT_ATAN2 = _op(95, 2)           # r_dest = (float-atan2 r_src0 r_src1)
    FLOAT_COSH = _op(96, 1)            # r_dest = (float-cosh r_src0)
    FLOAT_SINH = _op(97, 1)            # r_dest = (float-sinh r_src0)
    FLOAT_TANH = _op(98, 1)            # r_dest = (float-tanh r_src0)
    FLOAT_ASIN = _op(99, 1)            # r_dest = (float-asin r_src0)
    FLOAT_ACOS = _op(100, 1)            # r_dest = (float-acos r_src0)
    FLOAT_ATAN = _op(101, 1)            # r_dest = (float-atan r_src0)
    FLOAT_HYPOT = _op(102, 2)           # r_dest = (float-hypot r_src0 r_src1)
    FLOAT_EXP2 = _op(103, 1)            # r_dest = (float-exp2 r_src0)
    FLOAT_CBRT = _op(104, 1)            # r_dest = (float-cbrt r_src0)
    FLOAT_EXPM1 = _op(105, 1)           # r_dest = (float-expm1 r_src0)
    FLOAT_LOG1P = _op(106, 1)           # r_dest = (float-log1p r_src0)
    FLOAT_TRUNC = _op(107, 1)           # r_dest = (float-trunc r_src0)
    FLOAT_COPYSIGN = _op(108, 2)        # r_dest = (float-copysign r_src0 r_src1)

    # Complex operations
    COMPLEX_P = _op(109, 1)             # r_dest = (complex? r_src0)
    COMPLEX_EQ_P = _op(110, 2)          # r_dest = (complex=? r_src0 r_src1)
    COMPLEX_NEQ_P = _op(111, 2)         # r_dest = (complex!=? r_src0 r_src1)
    COMPLEX_REAL = _op(112, 1)          # r_dest = (complex-real r_src0)
    COMPLEX_IMAG = _op(113, 1)          # r_dest = (complex-imag r_src0)
    COMPLEX_ABS = _op(114, 1)           # r_dest = (complex-abs r_src0)
    COMPLEX_ADD = _op(115, 2)           # r_dest = (complex+ r_src0 r_src1)
    COMPLEX_SUB = _op(116, 2)           # r_dest = (complex- r_src0 r_src1)
    COMPLEX_MUL = _op(117, 2)           # r_dest = (complex* r_src0 r_src1)
    COMPLEX_DIV = _op(118, 2)           # r_dest = (complex/ r_src0 r_src1)
    COMPLEX_NEG = _op(119, 1)           # r_dest = (complex-neg r_src0)
    COMPLEX_EXP = _op(120, 1)           # r_dest = (complex-exp r_src0)
    COMPLEX_EXPN = _op(121, 2)          # r_dest = (complex-expn r_src0 r_src1)
    COMPLEX_LOG = _op(122, 1)           # r_dest = (complex-log r_src0)
    COMPLEX_LOG10 = _op(123, 1)         # r_dest = (complex-log10 r_src0)
    COMPLEX_LOGN = _op(124, 2)          # r_dest = (complex-logn r_src0 r_src1)
    COMPLEX_SIN = _op(125, 1)           # r_dest = (complex-sin r_src0)
    COMPLEX_COS = _op(126, 1)           # r_dest = (complex-cos r_src0)
    COMPLEX_TAN = _op(127, 1)           # r_dest = (complex-tan r_src0)
    COMPLEX_SQRT = _op(128, 1)          # r_dest = (complex-sqrt r_src0)
    COMPLEX_TO_STRING = _op(129, 1)     # r_dest = (complex->string r_src0)

    # String operations
    STRING_P = _op(130, 1)              # r_dest = (string? r_src0)
    STRING_EQ_P = _op(131, 2)           # r_dest = (string=? r_src0 r_src1)
    STRING_NEQ_P = _op(132, 2)          # r_dest = (string!=? r_src0 r_src1)
    STRING_LT_P = _op(133, 2)           # r_dest = (string<? r_src0 r_src1)
    STRING_GT_P = _op(134, 2)           # r_dest = (string>? r_src0 r_src1)
    STRING_LTE_P = _op(135, 2)          # r_dest = (string<=? r_src0 r_src1)
    STRING_GTE_P = _op(136, 2)          # r_dest = (string>=? r_src0 r_src1)
    STRING_LENGTH = _op(137, 1)         # r_dest = (string-length r_src0)
    STRING_UPCASE = _op(138, 1)         # r_dest = (string-upcase r_src0)
    STRING_DOWNCASE = _op(139, 1)       # r_dest = (string-downcase r_src0)
    STRING_TRIM = _op(140, 1)           # r_dest = (string-trim r_src0)
    STRING_TRIM_LEFT = _op(141, 1)      # r_dest = (string-trim-left r_src0)
    STRING_TRIM_RIGHT = _op(142, 1)     # r_dest = (string-trim-right r_src0)
    STRING_TO_INTEGER = _op(143, 2)     # r_dest = (string->integer r_src0 r_src1)
    STRING_TO_FLOAT = _op(144, 1)       # r_dest = (string->float r_src0)
    STRING_TO_LIST = _op(145, 2)        # r_dest = (string->list r_src0 r_src1)
    STRING_REF = _op(146, 2)            # r_dest = (string-ref r_src0 r_src1)
    STRING_PREFIX_P = _op(147, 2)       # r_dest = (string-prefix? r_src0 r_src1)
    STRING_SUFFIX_P = _op(148, 2)       # r_dest = (string-suffix? r_src0 r_src1)
    STRING_CONCAT = _op(149, 2)         # r_dest = (string-concat r_src0 r_src1)
    STRING_SLICE = _op(150, 3)          # r_dest = (string-slice r_src0 r_src1 r_src2)
    STRING_REPLACE = _op(151, 3)        # r_dest = (string-replace r_src0 r_src1 r_src2)
    STRING_INDEX = _op(152, 2)          # r_dest = (string-index r_src0 r_src1)
    STRING_TO_INTEGER_CODEPOINT = _op(153, 1)
    ...
    DICT_NEQ_P = _op(158, 2)            # r_dest = (dict!=? r_src0 r_src1)
    DICT_KEYS = _op(159, 1)             # r_dest = (dict-keys r_src0)
    DICT_VALUES = _op(160, 1)           # r_dest = (dict-values r_src0)
    DICT_LENGTH = _op(161, 1)           # r_dest = (dict-length r_src0)
    DICT_HAS_P = _op(162, 2)            # r_dest = (dict-has? r_src0 r_src1)
    DICT_REMOVE = _op(163, 2)           # r_dest = (dict-remove r_src0 r_src1)
    DICT_MERGE = _op(164, 2)            # r_dest = (dict-merge r_src0 r_src1)
    DICT_SET = _op(165, 3)              # r_dest = (dict-set r_src0 r_src1 r_src2)
    DICT_GET = _op(166, 3)              # r_dest = (dict-get r_src0 r_src1 r_src2)

    # List operations
    MAKE_LIST = _op(167, 2)             # r_dest = MAKE_LIST src0, src1 — base slot of outgoing zone, element count
    LIST_P = _op(168, 1)                # r_dest = (list? r_src0)
    LIST_EQ_P = _op(169, 2)             # r_dest = (list=? r_src0 r_src1)
    LIST_NEQ_P = _op(170, 2)            # r_dest = (list!=? r_src0 r_src1)
    LIST_PREPEND = _op(171, 2)          # r_dest = (list-prepend r_src0 r_src1)
    LIST_APPEND = _op(172, 2)           # r_dest = (list-append r_src0 r_src1)
    LIST_REVERSE = _op(173, 1)          # r_dest = (list-reverse r_src0)
    LIST_FIRST = _op(174, 1)            # r_dest = (list-first r_src0)
    LIST_REST = _op(175, 1)             # r_dest = (list-rest r_src0)
    LIST_LAST = _op(176, 1)             # r_dest = (list-last r_src0)
    LIST_LENGTH = _op(177, 1)           # r_dest = (list-length r_src0)
    LIST_REF = _op(178, 2)              # r_dest = (list-ref r_src0 r_src1)
    LIST_NULL_P = _op(179, 1)           # r_dest = (list-null? r_src0)
    LIST_MEMBER_P = _op(180, 2)         # r_dest = (list-member? r_src0 r_src1)
    LIST_INDEX = _op(181, 2)            # r_dest = (list-index r_src0 r_src1)
    LIST_SLICE = _op(182, 3)            # r_dest = (list-slice r_src0 r_src1 r_src2)
    LIST_REMOVE = _op(183, 2)           # r_dest = (list-remove r_src0 r_src1)
    LIST_CONCAT = _op(184, 2)           # r_dest = (list-concat r_src0 r_src1)
    LIST_TO_STRING = _op(185, 2)        # r_dest = (list->string r_src0 r_src1)
    LIST_TO_SET = _op(186, 1)           # r_dest = (list->set r_src0)

    # Set operations
    MAKE_SET = _op(187, 2)              # r_dest = MAKE_SET src0, src1 — base slot of outgoing zone, element count
    SET_P = _op(188, 1)                 # r_dest = (set? r_src0)
    SET_EQ_P = _op(189, 2)              # r_dest = (set=? r_src0 r_src1)
    SET_NEQ_P = _op(190, 2)             # r_dest = (set!=? r_src0 r_src1)
    SET_MEMBER_P = _op(191, 2)          # r_dest = (set-member? r_src0 r_src1)
    SET_ADD = _op(192, 2)               # r_dest = (set-add r_src0 r_src1)
    SET_REMOVE = _op(193, 2)            # r_dest = (set-remove r_src0 r_src1)
    SET_LENGTH = _op(194, 1)            # r_dest = (set-length r_src0)
    SET_UNION = _op(195, 2)             # r_dest = (set-union r_src0 r_src1)
    SET_INTERSECTION = _op(196, 2)      # r_dest = (set-intersection r_src0 r_src1)
    SET_DIFFERENCE = _op(197, 2)        # r_dest = (set-difference r_src0 r_src1)
    SET_SUBSET_P = _op(198, 2)          # r_dest = (set-subset? r_src0=superset r_src1=subset)
    SET_TO_LIST = _op(199, 1)           # r_dest = (set->list r_src0)

    # Struct operations
    MAKE_STRUCT = _op(200, 2)           # MAKE_STRUCT type_idx arity — pops arity field regs, pushes MenaiStruct
    STRUCT_P = _op(201, 1)              # r_dest = (struct? r_src0)
    STRUCT_IS_INSTANCE_P = _op(202, 2)  # r_dest = (struct-is-instance? r_src0 r_src1) — tag check, src0=struct, src1=structtype
    STRUCT_GET = _op(203, 2)            # r_dest = (struct-get r_src0 field_idx) — indexed field access
    STRUCT_REF = _op(204, 2)            # r_dest = (struct-ref r_src0 field_idx) — indexed field access by immediate
    STRUCT_SET = _op(205, 3)            # r_dest = (struct-set r_src0 field_idx r_src1) — functional update
    STRUCT_SET_REF = _op(206, 3)        # r_dest = (struct-set-ref r_src0 field_idx r_src1) — functional update by immediate
    STRUCT_EQ_P = _op(207, 2)           # r_dest = (struct=? r_src0 r_src1)
    STRUCT_NEQ_P = _op(208, 2)          # r_dest = (struct!=? r_src0 r_src1)
    STRUCT_TYPE = _op(209, 1)           # r_dest = (struct-type r_src0) → MenaiStructType value
    STRUCTTYPE_P = _op(210, 1)          # r_dest = (structtype? r_src0)
    STRUCTTYPE_EQ_P = _op(211, 2)       # r_dest = (structtype=? r_src0 r_src1)
    STRUCTTYPE_NEQ_P = _op(212, 2)      # r_dest = (structtype!=? r_src0 r_src1)
    STRUCTTYPE_NAME = _op(213, 1)       # r_dest = (structtype-name r_src0) → string
    STRUCTTYPE_FIELDS = _op(214, 1)     # r_dest = (structtype-fields r_src0) → list of symbols

    # Generate integer range list
    RANGE = _op(215, 3)                 # r_dest = (range r_src0 r_src1 r_src2)

    # Bytes operations — type foundation
    BYTES_P = _op(216, 1)               # r_dest = (bytes? r_src0)
    BYTES_EQ_P = _op(217, 2)            # r_dest = (bytes=? r_src0 r_src1)
    BYTES_NEQ_P = _op(218, 2)           # r_dest = (bytes!=? r_src0 r_src1)
    BYTES_LENGTH = _op(219, 1)          # r_dest = (bytes-length r_src0)
    BYTES_REF = _op(220, 2)             # r_dest = (bytes-ref r_src0 r_src1)
    BYTES_APPEND_U8 = _op(221, 2)       # r_dest = (bytes-append-u8 r_src0 r_src1)
    LIST_TO_BYTES = _op(222, 1)         # r_dest = (list->bytes r_src0)
    BYTES_SLICE = _op(223, 3)           # r_dest = (bytes-slice r_src0 r_src1 r_src2)
    STRING_TO_BYTES = _op(224, 1)       # r_dest = (string->bytes r_src0)
    BYTES_TO_STRING = _op(225, 1)       # r_dest = (bytes->string r_src0)
    BYTES_TO_LIST = _op(226, 1)         # r_dest = (bytes->list r_src0)
    BYTES_TO_STRING_HEX = _op(227, 1)   # r_dest = (bytes->string-hex r_src0)
    STRING_HEX_TO_BYTES = _op(228, 1)  # r_dest = (string-hex->bytes r_src0)
    BYTES_CONCAT = _op(229, 2)          # r_dest = (bytes-concat r_src0 r_src1)
    BYTES_INDEX = _op(230, 2)           # r_dest = (bytes-index r_src0 r_src1)
    BYTES_INDEX_INT = _op(231, 2)       # r_dest = (bytes-index-int r_src0 r_src1)
    BYTES_LT_P = _op(232, 2)            # r_dest = (bytes<? r_src0 r_src1)
    BYTES_GT_P = _op(233, 2)            # r_dest = (bytes>? r_src0 r_src1)
    BYTES_LTE_P = _op(234, 2)           # r_dest = (bytes<=? r_src0 r_src1)
    BYTES_GTE_P = _op(235, 2)           # r_dest = (bytes>=? r_src0 r_src1)
    BYTES_READ_U8 = _op(236, 2)         # r_dest = (bytes-read-u8 r_src0 r_src1)
    BYTES_READ_U16_LE = _op(237, 2)     # r_dest = (bytes-read-u16-le r_src0 r_src1)
    BYTES_READ_U24_LE = _op(238, 2)     # r_dest = (bytes-read-u24-le r_src0 r_src1)
    BYTES_READ_U32_LE = _op(239, 2)     # r_dest = (bytes-read-u32-le r_src0 r_src1)
    BYTES_READ_U64_LE = _op(240, 2)     # r_dest = (bytes-read-u64-le r_src0 r_src1)
    BYTES_READ_U16_BE = _op(241, 2)     # r_dest = (bytes-read-u16-be r_src0 r_src1)
    BYTES_READ_U24_BE = _op(242, 2)     # r_dest = (bytes-read-u24-be r_src0 r_src1)
    BYTES_READ_U32_BE = _op(243, 2)     # r_dest = (bytes-read-u32-be r_src0 r_src1)
    BYTES_READ_U64_BE = _op(244, 2)     # r_dest = (bytes-read-u64-be r_src0 r_src1)
    BYTES_READ_I8 = _op(245, 2)         # r_dest = (bytes-read-i8 r_src0 r_src1)
    BYTES_READ_I16_LE = _op(246, 2)     # r_dest = (bytes-read-i16-le r_src0 r_src1)
    BYTES_READ_I24_LE = _op(247, 2)     # r_dest = (bytes-read-i24-le r_src0 r_src1)
    BYTES_READ_I32_LE = _op(248, 2)     # r_dest = (bytes-read-i32-le r_src0 r_src1)
    BYTES_READ_I64_LE = _op(249, 2)     # r_dest = (bytes-read-i64-le r_src0 r_src1)
    BYTES_READ_I16_BE = _op(250, 2)     # r_dest = (bytes-read-i16-be r_src0 r_src1)
    BYTES_READ_I24_BE = _op(251, 2)     # r_dest = (bytes-read-i24-be r_src0 r_src1)
    BYTES_READ_I32_BE = _op(252, 2)     # r_dest = (bytes-read-i32-be r_src0 r_src1)
    BYTES_READ_I64_BE = _op(253, 2)     # r_dest = (bytes-read-i64-be r_src0 r_src1)
    BYTES_APPEND_U16_LE = _op(254, 2)   # r_dest = (bytes-append-u16-le r_src0 r_src1)
    BYTES_APPEND_U16_BE = _op(255, 2)   # r_dest = (bytes-append-u16-be r_src0 r_src1)
    BYTES_APPEND_U24_LE = _op(256, 2)   # r_dest = (bytes-append-u24-le r_src0 r_src1)
    BYTES_APPEND_U24_BE = _op(257, 2)   # r_dest = (bytes-append-u24-be r_src0 r_src1)
    BYTES_APPEND_U32_LE = _op(258, 2)   # r_dest = (bytes-append-u32-le r_src0 r_src1)
    BYTES_APPEND_U32_BE = _op(259, 2)   # r_dest = (bytes-append-u32-be r_src0 r_src1)
    BYTES_APPEND_U64_LE = _op(260, 2)   # r_dest = (bytes-append-u64-le r_src0 r_src1)
    BYTES_APPEND_U64_BE = _op(261, 2)   # r_dest = (bytes-append-u64-be r_src0 r_src1)
    BYTES_APPEND_I8 = _op(262, 2)       # r_dest = (bytes-append-i8 r_src0 r_src1)
    BYTES_APPEND_I16_LE = _op(263, 2)   # r_dest = (bytes-append-i16-le r_src0 r_src1)
    BYTES_APPEND_I16_BE = _op(264, 2)   # r_dest = (bytes-append-i16-be r_src0 r_src1)
    BYTES_APPEND_I24_LE = _op(265, 2)   # r_dest = (bytes-append-i24-le r_src0 r_src1)
    BYTES_APPEND_I24_BE = _op(266, 2)   # r_dest = (bytes-append-i24-be r_src0 r_src1)
    BYTES_APPEND_I32_LE = _op(267, 2)   # r_dest = (bytes-append-i32-le r_src0 r_src1)
    BYTES_APPEND_I32_BE = _op(268, 2)   # r_dest = (bytes-append-i32-be r_src0 r_src1)
    BYTES_APPEND_I64_LE = _op(269, 2)   # r_dest = (bytes-append-i64-le r_src0 r_src1)
    BYTES_APPEND_I64_BE = _op(270, 2)   # r_dest = (bytes-append-i64-be r_src0 r_src1)
    BYTES_WRITE_U8 = _op(271, 3)        # r_dest = (bytes-write-u8 r_src0 r_src1 r_src2)
    BYTES_WRITE_U16_LE = _op(272, 3)    # r_dest = (bytes-write-u16-le r_src0 r_src1 r_src2)
    BYTES_WRITE_U16_BE = _op(273, 3)    # r_dest = (bytes-write-u16-be r_src0 r_src1 r_src2)
    BYTES_WRITE_U24_LE = _op(274, 3)    # r_dest = (bytes-write-u24-le r_src0 r_src1 r_src2)
    BYTES_WRITE_U24_BE = _op(275, 3)    # r_dest = (bytes-write-u24-be r_src0 r_src1 r_src2)
    BYTES_WRITE_U32_LE = _op(276, 3)    # r_dest = (bytes-write-u32-le r_src0 r_src1 r_src2)
    BYTES_WRITE_U32_BE = _op(277, 3)    # r_dest = (bytes-write-u32-be r_src0 r_src1 r_src2)
    BYTES_WRITE_U64_LE = _op(278, 3)    # r_dest = (bytes-write-u64-le r_src0 r_src1 r_src2)
    BYTES_WRITE_U64_BE = _op(279, 3)    # r_dest = (bytes-write-u64-be r_src0 r_src1 r_src2)
    BYTES_WRITE_I8 = _op(280, 3)        # r_dest = (bytes-write-i8 r_src0 r_src1 r_src2)
    BYTES_WRITE_I16_LE = _op(281, 3)    # r_dest = (bytes-write-i16-le r_src0 r_src1 r_src2)
    BYTES_WRITE_I16_BE = _op(282, 3)    # r_dest = (bytes-write-i16-be r_src0 r_src1 r_src2)
    BYTES_WRITE_I24_LE = _op(283, 3)    # r_dest = (bytes-write-i24-le r_src0 r_src1 r_src2)
    BYTES_WRITE_I24_BE = _op(284, 3)    # r_dest = (bytes-write-i24-be r_src0 r_src1 r_src2)
    BYTES_WRITE_I32_LE = _op(285, 3)    # r_dest = (bytes-write-i32-le r_src0 r_src1 r_src2)
    BYTES_WRITE_I32_BE = _op(286, 3)    # r_dest = (bytes-write-i32-be r_src0 r_src1 r_src2)
    BYTES_WRITE_I64_LE = _op(287, 3)    # r_dest = (bytes-write-i64-le r_src0 r_src1 r_src2)
    BYTES_WRITE_I64_BE = _op(288, 3)    # r_dest = (bytes-write-i64-be r_src0 r_src1 r_src2)
    BYTES_READ_ULEB128 = _op(289, 2)    # r_dest = (bytes-read-uleb128 r_src0 r_src1)
    BYTES_APPEND_ULEB128 = _op(290, 2)  # r_dest = (bytes-append-uleb128 r_src0 r_src1)
    BYTES_READ_SLEB128 = _op(291, 2)    # r_dest = (bytes-read-sleb128 r_src0 r_src1)
    BYTES_APPEND_SLEB128 = _op(292, 2)  # r_dest = (bytes-append-sleb128 r_src0 r_src1)

    # Type guard opcodes — assert runtime type, raise MENAI_ERR_TYPE_MISMATCH if wrong.
    ASSERT_NONE = _op(293, 1)           # Check r_src0 — assert r_src0 is none
    ASSERT_BOOLEAN = _op(294, 1)        # Check r_src0 — assert r_src0 is boolean
    ASSERT_INTEGER = _op(295, 1)        # Check r_src0 — assert r_src0 is integer
    ASSERT_FLOAT = _op(296, 1)          # Check r_src0 — assert r_src0 is float
    ASSERT_COMPLEX = _op(297, 1)        # Check r_src0 — assert r_src0 is complex
    ASSERT_STRING = _op(298, 1)         # Check r_src0 — assert r_src0 is string
    ASSERT_SYMBOL = _op(299, 1)         # Check r_src0 — assert r_src0 is symbol
    ASSERT_LIST = _op(300, 1)           # Check r_src0 — assert r_src0 is list
    ASSERT_DICT = _op(301, 1)           # Check r_src0 — assert r_src0 is dict
    ASSERT_SET = _op(302, 1)            # Check r_src0 — assert r_src0 is set
    ASSERT_FUNCTION = _op(303, 1)       # Check r_src0 — assert r_src0 is function
