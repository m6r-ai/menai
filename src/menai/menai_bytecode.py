...
class Opcode(IntEnum):
    ...
    # List operations
    MAKE_LIST = _op(300, 2)             # r_dest = MAKE_LIST src0, src1 — base slot of outgoing zone, element count
    LIST_P = _op(301, 1)                # r_dest = (list? r_src0)
    LIST_EQ_P = _op(302, 2)             # r_dest = (list=? r_src0 r_src1)
    LIST_NEQ_P = _op(303, 2)            # r_dest = (list!=? r_src0 r_src1)
    LIST_PREPEND = _op(304, 2)          # r_dest = (list-prepend r_src0 r_src1)
    LIST_APPEND = _op(305, 2)           # r_dest = (list-append r_src0 r_src1)
    LIST_REVERSE = _op(306, 1)          # r_dest = (list-reverse r_src0)
    LIST_FIRST = _op(307, 1)            # r_dest = (list-first r_src0)
    LIST_REST = _op(308, 1)             # r_dest = (list-rest r_src0)
    LIST_LAST = _op(309, 1)             # r_dest = (list-last r_src0)
    LIST_LENGTH = _op(310, 1)           # r_dest = (list-length r_src0)
    LIST_REF = _op(311, 2)              # r_dest = (list-ref r_src0 r_src1)
    LIST_NULL_P = _op(312, 1)           # r_dest = (list-null? r_src0)
    LIST_MEMBER_P = _op(313, 2)         # r_dest = (list-member? r_src0 r_src1)
    LIST_INDEX = _op(314, 2)            # r_dest = (list-index r_src0 r_src1)
    LIST_SLICE = _op(315, 3)            # r_dest = (list-slice r_src0 r_src1 r_src2)
    LIST_REMOVE = _op(316, 2)           # r_dest = (list-remove r_src0 r_src1)
    LIST_CONCAT = _op(317, 2)           # r_dest = (list-concat r_src0 r_src1)
    LIST_TO_STRING = _op(318, 2)        # r_dest = (list->string r_src0 r_src1)
    LIST_TO_SET = _op(319, 1)           # r_dest = (list->set r_src0)

    # Set operations
    MAKE_SET = _op(340, 2)              # r_dest = MAKE_SET src0, src1 — base slot of outgoing zone, element count
    SET_P = _op(341, 1)                 # r_dest = (set? r_src0)
    SET_EQ_P = _op(342, 2)              # r_dest = (set=? r_src0 r_src1)
    SET_NEQ_P = _op(343, 2)             # r_dest = (set!=? r_src0 r_src1)
    SET_MEMBER_P = _op(344, 2)          # r_dest = (set-member? r_src0 r_src1)
    SET_ADD = _op(345, 2)               # r_dest = (set-add r_src0 r_src1)
    SET_REMOVE = _op(346, 2)            # r_dest = (set-remove r_src0 r_src1)
    SET_LENGTH = _op(347, 1)            # r_dest = (set-length r_src0)
