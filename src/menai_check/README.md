...
```bash
python -m menai_check.check file.menai
```
...
```bash
# Lines 100 to 200
python -m menai_check.check file.menai -l 100-200

# From line 100 to end
python -m menai_check.check file.menai -l 100-

# From start to line 200
python -m menai_check.check file.menai -l -200
```
...
python -m menai_check.check file.menai -l 100-200 -a

# Quick summary check
python -m menai_check.check file.menai -s
```
...
```bash
python -m menai_check.check file.menai -a -c
```
...
```bash
$ python -m menai_check.check examples/balanced.menai
...
```bash
$ python -m menai_check.check examples/missing_close.menai
...
```bash
$ python -m menai_check.check scheduling.menai -a -l 395-402
...
for file in $(git diff --cached --name-only --diff-filter=ACM | grep '\.menai$'); do
    python -m menai_check.check "$file"
    if [ $? -ne 0 ]; then
      ...
      - name: Check Menai parentheses
        run: |
          for file in $(find . -name "*.menai"); do
            python -m menai_check.check "$file" || exit 1
          done
```
