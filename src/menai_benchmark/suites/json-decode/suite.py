...
def _to_menai_expr(json_str: str) -> str:
    """Wrap a JSON string in a Menai decode call, escaping for Menai string syntax."""
    escaped = json_str.replace("\\", "\\\\").replace('"', '\\"')
    return f'(let ((json (import "json-decode"))) ((:: json decode) "{escaped}"))'


class Suite(BenchmarkSuite):
    """Benchmark suite for the Menai JSON decoder."""

    name = "json-decode"
    description = "Decode JSON strings of varying structure and size."
