"""
Generate the zlib fixture files used by the zlib_parser benchmark suite.

Each fixture is a complete zlib stream (RFC 1950) whose decompressed size and
content variety exercise the inflate paths inside zlib_parser.  The fixtures
are committed so every run reads byte-identical inputs.

Run from the repository root:

    python src/menai_benchmark/suites/zlib_parser/generate_fixtures.py
